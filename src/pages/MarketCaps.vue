<template>
    <div id="market-caps">
        ...
        <hot-table :data="tableData" :settings="settings" licenseKey="non-commercial-and-evaluation"
                   ref="table"
        >
            <hot-column title="Symbol" data="coinSymbol"></hot-column>
...
export default {
    ...
    methods: {
        async reloadTable() {
            ...
            const priceAtMCFraction = (fraction, circulatingSupply) =>
                baseCurrencySymbol + (fraction * bitcoin.market_cap / circulatingSupply).toFixed(2);
            ...
            const newTableData = map(coinData, (coin) => {
                ...
                return {
                    ...
                    price               : baseCurrencySymbol + (coin.current_price).toFixed(2),
                    fromAth             : Math.round(coin.ath_change_percentage) + "%",
                    priceChange24h      : Math.round(coin.price_change_percentage_24h_in_currency) + "%",
                    priceChange7d       : Math.round(coin.price_change_percentage_7d_in_currency) + "%",
                    priceChange30d      : Math.round(coin.price_change_percentage_30d_in_currency) + "%",
                    priceChange200d     : Math.round(coin.price_change_percentage_200d_in_currency) + "%",

                    marketCap           : baseCurrencySymbol + coin.market_cap,
                    marketCapRank       : coin.market_cap_rank,

                    btcMcFraction       : ((coin.market_cap / bitcoin.market_cap) * 100).toFixed(2) + "%",
